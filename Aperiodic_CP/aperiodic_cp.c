...
void fibonacci_word(int k, int Size, int Fibb[], int offset) {
	int i, j, pos, end, FibbTemp[Size + offset];
	FibbTemp[0] = 0;
	for(i=1; i <= k; i++) {
		FibbTemp[i] = 1;
	}
...
int main(int argc, char **argv) {
	...
	int k, point,  pos, list_pos, list_last, sim, sim_init, sim_final, Size, offset;
	int *Lattice, *InfecList, *Vizd, *Vize, *Fibb;
	double la, lb, prob, pleft, pright, pla, pra, plb, prb, t, tmax, rho, rho_0, time_spent, Times[Points];
	FILE *output_file;
	...
	offset = rand()%(int)tmax;
	fibonacci_word(k, tmax, Fibb, offset);

//	for(sim = 0; sim < tmax; sim++)
//		printf("%d", Fibb[sim]);
//	printf("\n");
//	return 0;
